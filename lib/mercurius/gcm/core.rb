...
module GCM
  ...
  class << self
    ...
    def key(identity = nil)
      if @key.is_a?(Hash)
        raise %{If your key is a hash of keys you'll need to pass an identifier to the notification!} if identity.nil?
        return @key[identity]
      else
        return @key
      end
  ...
  def self.send_notifications(notifications)
    [].tap do |responses|
      notifications.each do |notification|
        responses << self.prepare_and_send(notification)
      end
    end
  end
